...
\begin{lstlisting}[caption={Výhody třídy $Enum$},label={lst:enum-pros}]
...
\begin{sloppypar}
    ...
    \fullref{lst:map-indexing} znázorňuje přetížené indexování instancí třídy \ic|Map| pomocí pozice uložené v~vestavěném datovém typu \ic|tuple| (nebo jakýkoliv jiný objekt, který implementuje metodu \ic|__iter__|). Výsledkem je instance prázdného herního pole.
\end{sloppypar}
